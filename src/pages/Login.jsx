...
const Login = () => {
  ...
  return (
    <div className='overflow-hidden w-full relative h-screen'>
      <img className='w-full h-1/3 object-cover' src="/images/banner2.webp" alt="" />
      <div className='w-full h-full bg-[#000000b9] absolute top-0'></div> 
      <h2 className='text-white absolute z-30 text-5xl tracking-tighter font-[rome] italic top-[15%] left-1/2 transform translate-x-[-50%] opacity-90'>Login</h2>
      ...
      <div className="w-full flex items-center justify-center px-6 bg-white h-[75vh] z-40 absolute bottom-0 rounded-l-[50px]">
        <form onSubmit={handleSubmit(submitHandler)} className='border-black w-full h-[70%] flex flex-col items-center gap-8'>
          ...
          <button type='submit' className='bg-[#ed4c22] rounded px-4 py-2 text-xl font-semibold w-1/3 mx-auto'>Login</button>

          <small className='text-black text-sm font-semibold'>Don't have an account ? <Link to={"/register"}><span className='text-[#ed4c22]'>Register</span></Link></small>
        </form>
